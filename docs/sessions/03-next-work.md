...
| 2 | #11 | First Alembic revision (stop relying only on `create_all`) — **next** |
...
Already partially landed (keep issues open until acceptance fully met): #13 Chroma wrapper (branch, unmerged), #11 models/`db-init`, #12 write queue, #14 stub embedder, #15 chat adapters, #18 allowlist handlers, #21 basic routes.
