...
- Ingest pipeline (#16) assigns `chunks.chroma_id` as `c{capture_id}-n{ordinal}` and upserts when a `VectorStore` is attached to the pipeline. `juno serve` still passes `vectors=None` until #13 merges.
